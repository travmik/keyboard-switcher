...
- **Replacement is paste-only, single writer** (amended 2026-09-14): the AX write is not used at all. Two findings forced this: WebKit apps (ChatGPT) report AX-write success without applying it, and Telegram applies AX writes *asynchronously* — later than any verification re-read — so a paste fallback on top of a "failed" write duplicated the text. With the paste as the only writer, double application is structurally impossible. Cost: the clipboard always ends up holding the translated text (restore is a Settings toggle, off by default; restoring can still race in slow-paste apps).
- **Paste**: save clipboard string → copy translated text → post synthetic ⌘V → restore per setting. The synthetic keystroke is posted as a full physical stream — ⌘ down (`flagsChanged`), key down, key up, ⌘ up (`kVK_Command` modifier transitions) — because some editors track modifier state transitions and drop key equivalents that appear without them.
- **ChatGPT-class exception**: the ChatGPT desktop app (bundle IDs `com.openai.chat`/`com.openai.codex`) ignores synthetic input entirely — ⌘V at session and HID level, full modifier streams, and unicode-string key events all fail, while synthetic ⌘C works. For it, the Paste menu item of the frontmost app is **pressed via AX** (`AXUIElementPerformAction(kAXPressAction)`), found by traversing the menu bar and matching `AXMenuItemCmdChar` == "V" with Command-only modifiers (localization-independent). Menu actions are dispatched by the app itself and are immune to the event-delivery problem.
