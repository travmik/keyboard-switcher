...
- **AX write verification** (amended 2026-09-09): WebKit-based apps (ChatGPT desktop) report AX-write success without applying it. After every AX write, the selection is re-read: equals the written text or is empty (selection collapsed) → success; still holds other text → the write was a no-op, fall through to the paste path.
- **Paste fallback**: save clipboard string → copy translated text → post synthetic ⌘V → restore per setting. The synthetic keystroke is posted as a full physical stream — ⌘ down (`flagsChanged`), key down, key up, ⌘ up (`kVK_Command` modifier transitions) — because some editors track modifier state transitions and drop key equivalents that appear without them.
